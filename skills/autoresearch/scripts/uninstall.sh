...
BACKUP_FILE="${OPENCODE_BASE}/skills/autoresearch/scripts/backup-state.sh"
ENFORCE_FILE="${OPENCODE_BASE}/skills/autoresearch/scripts/enforce-autoresearch-state.sh"
BACKUP_USAGE_FILE="${OPENCODE_BASE}/skills/autoresearch/BACKUP-USAGE.md"
...
if [[ -f "${ENFORCE_FILE}" ]]; then
    EXISTING_FILES+=("${ENFORCE_FILE}")
fi

if [[ -f "${BACKUP_USAGE_FILE}" ]]; then
    EXISTING_FILES+=("${BACKUP_USAGE_FILE}")
fi
...
# Remove enforcement script
if [[ -f "${ENFORCE_FILE}" ]]; then
    if rm -f "${ENFORCE_FILE}" 2>/dev/null; then
        REMOVED_FILES+=("${ENFORCE_FILE}")
        echo "   ✓ Removed: ${ENFORCE_FILE}"
    else
        FAILED_REMOVALS+=("${ENFORCE_FILE}")
        echo "   ✗ Failed to remove: ${ENFORCE_FILE}"
    fi
fi

# Remove backup usage documentation
if [[ -f "${BACKUP_USAGE_FILE}" ]]; then
    if rm -f "${BACKUP_USAGE_FILE}" 2>/dev/null; then
        REMOVED_FILES+=("${BACKUP_USAGE_FILE}")
        echo "   ✓ Removed: ${BACKUP_USAGE_FILE}"
    else
        FAILED_REMOVALS+=("${BACKUP_USAGE_FILE}")
        echo "   ✗ Failed to remove: ${BACKUP_USAGE_FILE}"
    fi
fi
