...
# Go up two levels from scripts/autoresearch/scripts to get project root
...
REQUIRED_FILES=(
    ...
    "skills/autoresearch/BACKUP-USAGE.md"
...
INSTALL_MAP=(
    ...
    ["skills/autoresearch/BACKUP-USAGE.md"]="${OPENCODE_CONFIG}/skills/autoresearch/BACKUP-USAGE.md"
...
BACKUP_DEST="${OPENCODE_CONFIG}/skills/autoresearch/scripts/backup-state.sh"

if [[ -f "${BACKUP_SCRIPT}" ]]; then
    mkdir -p "${OPENCODE_CONFIG}/skills/autoresearch/scripts"
...
ENFORCE_SCRIPT="${SCRIPT_DIR}/enforce-autoresearch-state.sh"
ENFORCE_DEST="${OPENCODE_CONFIG}/skills/autoresearch/scripts/enforce-autoresearch-state.sh"

if [[ -f "${ENFORCE_SCRIPT}" ]]; then
    mkdir -p "${OPENCODE_CONFIG}/skills/autoresearch/scripts"
    if cp "${ENFORCE_SCRIPT}" "${ENFORCE_DEST}"; then
        chmod +x "${ENFORCE_DEST}"
        print_success "enforce-autoresearch-state.sh: Installed to ${ENFORCE_DEST}"
    else
        print_error "Failed to copy enforce-autoresearch-state.sh"
        INSTALL_FAILED=true
    fi
else
    print_warning "enforce-autoresearch-state.sh not found in project root, skipping enforcement script installation"
fi

# Install backup usage documentation (same dir as SKILL.md)
BACKUP_USAGE="${PROJECT_ROOT}/skills/autoresearch/BACKUP-USAGE.md"
BACKUP_USAGE_DEST="${OPENCODE_CONFIG}/skills/autoresearch/BACKUP-USAGE.md"

if [[ -f "${BACKUP_USAGE}" ]]; then
    if cp "${BACKUP_USAGE}" "${BACKUP_USAGE_DEST}"; then
        print_success "BACKUP-USAGE.md: Installed to ${BACKUP_USAGE_DEST}"
    else
        print_error "Failed to copy BACKUP-USAGE.md"
        INSTALL_FAILED=true
    fi
else
    print_warning "BACKUP-USAGE.md not found, skipping documentation installation"
fi
...
echo ""
echo "  ${GREEN}✓${NC} enforce-autoresearch-state.sh"
echo "    Installed to: ${ENFORCE_DEST}"
echo ""
echo "  ${GREEN}✓${NC} BACKUP-USAGE.md"
echo "    Installed to: ${BACKUP_USAGE_DEST}"
